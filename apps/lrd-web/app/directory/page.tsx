      ...
      <main style={{ minHeight: '100vh', backgroundColor: 'var(--color-background)' }}>
        ...
        <div className="py-10 border-b" style={{ borderColor: 'var(--color-border)' }}>
          <div className="container-fluid">
            <h1 className="text-3xl font-bold mb-2" style={{ fontFamily: 'var(--font-heading)' }}>Répertoire des experts</h1>
        ...
        <div className="container-fluid py-8">
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-6">
            ...
            <button
              ...
              style={{ backgroundColor: 'var(--color-muted)', border: '1px solid var(--color-border)', color: 'var(--color-foreground)' }}
            >
              <Filter size={16} /> Filtres {(selectedDomain || availabilityFilter) && <span className="w-2 h-2 rounded-full" style={{ backgroundColor: 'var(--color-accent)' }} />}
            </button>
          </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {DOMAINS.map(d => (
                    <button key={d} onClick={() => setSelectedDomain(selectedDomain === d ? '' : d)}
                      ...
                      style={{
                        backgroundColor: selectedDomain === d ? 'var(--color-accent)' : 'var(--color-background)',
                        color: selectedDomain === d ? 'white' : 'var(--color-foreground)',
                ...
                <div className="flex gap-2">
                  {[{ v: 'available', l: 'Disponible' }, { v: 'busy', l: 'Occupé' }].map(o => (
                    <button key={o.v} onClick={() => setAvailabilityFilter(availabilityFilter === o.v ? '' : o.v)}
                      className="px-3 py-1 rounded-full text-xs font-medium transition-all"
                      style={{
                        backgroundColor: availabilityFilter === o.v ? 'var(--color-primary)' : 'var(--color-background)',
                        color: availabilityFilter === o.v ? 'var(--color-on-primary)' : 'var(--color-foreground)',
                        border: '1px solid var(--color-border)'
                      }}
                    >{o.l}</button>
                  ))}
          ...
          {(selectedDomain || availabilityFilter) && (
            <div className="flex gap-2 mb-4 flex-wrap">
              {selectedDomain && (
                <span className="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium" style={{ backgroundColor: 'rgba(161,98,7,0.1)', color: 'var(--color-accent)' }}>
                  {selectedDomain} <button onClick={() => setSelectedDomain('')}><X size={10} /></button>
                </span>
              )}
              {availabilityFilter && (
                <span className="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium" style={{ backgroundColor: 'rgba(161,98,7,0.1)', color: 'var(--color-accent)' }}>
                  {availabilityFilter === 'available' ? 'Disponible' : 'Occupé'} <button onClick={() => setAvailabilityFilter('')}><X size={10} /></button>
                </span>
              )}
            </div>
          )}
            ...
            <div className="flex justify-center gap-2 mt-10">
              ...
              {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setPage(p)}
                  ...
                  style={{
                    backgroundColor: page === p ? 'var(--color-accent)' : 'transparent',
                    color: page === p ? 'white' : 'var(--color-foreground)',
                    border: '1px solid var(--color-border)'
                  }}
                >{p}</button>
              ))}
